...
class LSegEnc(BaseModel):
    ...
    def forward(self, x, labelset):
        ...
        # normalized features
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)  # text/class embeddings generated

        pixel_encoding = self.logit_scale * image_features.half()   # dot product to generate pixel embeddings
