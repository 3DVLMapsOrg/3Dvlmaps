...
# gs is the grid size, number of cells in x and y of top-down map
# cs is the cell size, real-world distance each cell represents
# xx is the x coordinate of a point in the world
# yy is the y coordinate of a point in the world (meters)

# equation (1) in page 3 of VLmaps for Robot Navigation p_x_map and p_y_map

def pos2grid_id(gs, cs, xx, yy):
    x = int(gs / 2 + int(xx / cs)) # pxmap center of grid along x-axis + real-world x converted to cell x
    y = int(gs / 2 - int(yy / cs)) # pymap
    return [x, y]


# SO OUR PROJECT IDEA IS BASICALLY WE ARE REPLACING pos2grid_id method with VoxelGrid-create_from_point_cloud method


# MAKE OUR OWN VOXEL TRANSFORMATION FUNCTION
...
def grid_id2pos(gs, cs, x, y):
    xx = (x - gs / 2) * cs #cell coordinates back to real-world
    zz = (gs / 2 - y) * cs
...
def project_point(cam_mat, p):
    new_p = cam_mat @ p.reshape((3, 1)) #cam_mat is a camera calibration matrix, p is the 3D point
    z = new_p[2, 0]
    new_p = new_p / new_p[2, 0] #normalizes the homogeneous coordinates by depth value
    x = int(new_p[0, 0] + 0.5) #image plane coordinate conversion
    y = int(new_p[1, 0] + 0.5)
    return x, y, z
